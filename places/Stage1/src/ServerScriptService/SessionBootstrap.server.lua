...
local function debugPrintJoinData(plr: Player, joinData: any?)
	if typeof(joinData) ~= "table" then
		print("[Stage1 SessionBootstrap]", plr.Name, "JoinData missing")
		return
	end
	...
	print(string.format(
		"[Stage1 SessionBootstrap] JobId=%s PlaceId=%d Player=%s sessionId=%s privateCode=%s fromPlaceId=%s reason=%s userRole=%s roomCode=%s",
		game.JobId,
		game.PlaceId,
		plr.Name,
		tostring(sid),
