...
--!strict
-- ReplicatedFirst.QuizHudBus (BindableEvent)를 받아서
...
-- ========= HUDGui 구조 해석 =========
local function buildUIRefs(gui: ScreenGui)
	...
	local bg    = frame:FindFirstChild("BackGround", true)
	local barBG = bg and bg:FindFirstChild("BarBackGround", true)
	local bar   = barBG and barBG:FindFirstChild("Bar", true)
	local txt   = frame:FindFirstChild("ProgressText", true)
...
-- ========= 전체 화면 플래시 =========
local function getOrCreateFlashOverlay()
...
-- ========= 정답 동그라미 링 =========
local function getOrCreateResultRing()
	local u = ensureUI(); if not u then return nil end
	local gui = u.gui
