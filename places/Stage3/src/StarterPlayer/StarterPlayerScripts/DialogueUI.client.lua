...
-- ★ 돌고래 초상 이미지
local DOLPHIN_PORTRAIT_ID = "rbxassetid://128763543129712"
...
local SFX_IDS: {[string]: string} = {
	...
	DialogueStart = "rbxassetid://9116302511",     -- 캐릭터/대화 등장
...
local function playPortalCutsceneAndWait()
	...
	pcall(function()
		spawnedPortal = PortalSpawnCutscene.play({
			templateName   = "Potal",
			parent         = Workspace,
			targetPosition = Vector3.new(421.158, 27.69, -330.698),
			camStartPath   = "CamPos",
...
local function runDialogueSequence(npc: Instance?, src:any, questPhase:number?)
	...
	local function handleAdvance(playNextSound: boolean)
		...
		if (not isHintLine)
			...
			and string.find(currentText, "다음 구역으로 가는 포탈이다", 1, true) ~= nil
		then
...
bus.Event:Connect(function(cmd:any, payload:any)
	...
	elseif cmd == "Close" or cmd == "Finished" then
		...
		if bg then
			bg.Visible = false -- ✅ Close도 BG로 닫기
		end
