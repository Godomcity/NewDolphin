...
--!strict
-- (Stage1 템플릿 기반, Stage3에서도 재사용 가능)
-- 선생님으로 들어오면:
--  - 퀴즈/퀘스트 관련 프롬프트 비활성화
--  - 쓰레기/정리 대상 오브젝트 로컬에서 정리 (InteractionActionRouter 사용)
--  - 문은 이미 열린 상태 (PortalMover.Open/FadeOut)
--  - NPC를 최종 위치로 이동
--  - 포탈은 이미 나와 있는 상태로만 보여줌 (컷씬/카메라 연출 없이)
--  - ★ (추가) JellyfishCutscene.PlayJellyOnly 로 젤리 물방울 연출만 재생
...
-- ★ 새로 사용: JellyfishCutscene (Stage3 젤리 연출용 모듈)
local JellyfishCutscene =
...
local function spawnPortalForTeacher(): Instance?
	...
	local targetPos = Vector3.new(421.158, 27.69, -330.698)
...
----------------------------------------------------------------
-- 5) (추가) 선생님 입장 시 젤리 물방울 연출만 재생
--     - JellyfishCutscene.PlayJellyOnly 사용 (카메라는 유지)
----------------------------------------------------------------
local function playJellyForTeacher()
	if not JellyfishCutscene or typeof(JellyfishCutscene.PlayJellyOnly) ~= "function" then
		warn("[StageTeacherSkip_Stage1] JellyfishCutscene.PlayJellyOnly 가 없습니다")
		return
	end

	-- 기본 설정: 모듈 안에서 Stage3/JellyFish 를 찾아서 애니메이션 재생
	local ok, err = pcall(function()
		JellyfishCutscene.PlayJellyOnly({
			jelly = nil,      -- nil이면 모듈 안에서 getJelly() 사용
			onFinished = nil, -- 필요하면 콜백 추가 가능
		})
	end)

	if not ok then
		warn("[StageTeacherSkip_Stage1] playJellyForTeacher error:", err)
	else
		print("[StageTeacherSkip_Stage1] JellyfishCutscene.PlayJellyOnly called for teacher")
	end
end
...
task.defer(function()
	...
	-- 4) 선생님 입장 시 젤리 물방울 연출만 한 번 재생 (카메라 고정 버전)
	playJellyForTeacher()

	-- 5) 포탈 템플릿을 바로 스폰 (컷씬 없이)
	local portal = spawnPortalForTeacher()

	-- 6) 가이드가 있다면 포탈 쪽으로 향하게
