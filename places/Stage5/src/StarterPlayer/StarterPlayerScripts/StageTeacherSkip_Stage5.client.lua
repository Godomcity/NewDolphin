...
local function applyDoorOpenedVisual()
	if not MiddleDoorCutscene or typeof(MiddleDoorCutscene.PlayLeafOnly) ~= "function" then
		warn("[StageTeacherSkip_Stage1] MiddleDoorCutscene.PlayLeafOnly 를 찾지 못했습니다. 문 오픈 스킵")
		return
	end

	local ok, err = pcall(function()
		MiddleDoorCutscene.PlayLeafOnly()
	end)
