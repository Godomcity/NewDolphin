...
-- 제리피쉬 컷씬 모듈
local cutScene =
	tryRequire(RS:FindFirstChild("Modules") and RS.Modules:FindFirstChild("WaterleafCutscene"))
	or tryRequire(RS:FindFirstChild("WaterleafCutscene"))
...
local RF_Stage5_GetProgress   = Remotes:WaitForChild("RF_Stage5_GetProgress")   :: RemoteFunction
local RE_Stage5_QuestSync     = Remotes:WaitForChild("RE_Stage5_QuestSync")     :: RemoteEvent
local RE_Stage5_CutsceneFlag  = Remotes:WaitForChild("RE_Stage5_CutsceneFlag")  :: RemoteEvent
local RE_Stage5_ObjectCleaned = Remotes:WaitForChild("RE_Stage5_ObjectCleaned") :: RemoteEvent
local RE_Stage5_QuizSolved    = Remotes:WaitForChild("RE_Stage5_QuizSolved")    :: RemoteEvent
...
local function syncQuestToServer()
	...
	if not RE_Stage5_QuestSync then return end

	local phase = STATE.QuestPhase.Value
	local extra = STATE.ExtraTrash.Value

	local ok, err = pcall(function()
		RE_Stage5_QuestSync:FireServer(phase, extra)
	end)
	if not ok then
	...
	conns[#conns+1] = UI.btnAnswer.MouseButton1Click:Connect(function()
		...
		if isCorrect then
			...
			task.delay(0.22, function()
				...
				if RE_Stage5_QuizSolved then
					RE_Stage5_QuizSolved:FireServer(qidStr)
				end
				...
				do
					...
					if cleanedObjectId and RE_Stage5_ObjectCleaned then
						RE_Stage5_ObjectCleaned:FireServer(cleanedObjectId)
					end
				end
				...
				if not cutsceneSent and solved >= CUTSCENE_THRESHOLD then
					...
					if RE_Stage5_CutsceneFlag then
						RE_Stage5_CutsceneFlag:FireServer("leaf_cut")
					end
...
local function loadStage3Progress()
	...
	local ok, res = pcall(function()
		return RF_Stage5_GetProgress:InvokeServer()
	end)
	if not ok or typeof(res) ~= "table" then
		warn("[Stage3][QuizClient] RF_Stage5_GetProgress failed:", res)
	...
	-------------------------------------------------------
	-- 컷씬 플래그 (워터리프 문 컷씬 재입장 처리)
	-------------------------------------------------------
	if CutsceneFlags["leaf_cut"] then
		print("[Stage3][QuizClient] leaf_cut already done → skip cutscene")
		STATE.ZoomPlayed.Value = true
		cutsceneSent = true
		...
		task.delay(1.0, function()
			if cutScene and typeof(cutScene.PlayLeafOnly) == "function" then
				pcall(function()
					cutScene.PlayLeafOnly()
				end)
			end
		end)
	end
