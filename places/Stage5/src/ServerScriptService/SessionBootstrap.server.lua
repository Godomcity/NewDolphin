...
-- + Stage5에서는 SessionResume에 "지금 Stage5에 있다" 라는 정보도 저장
...
local function debugPrintJoinData(plr: Player, joinData: any?)
        if typeof(joinData) ~= "table" then
                print("[Stage5 SessionBootstrap]", plr.Name, "JoinData missing")
                return
        end

        local td = joinData.TeleportData
        if typeof(td) ~= "table" then
                print("[Stage5 SessionBootstrap]", plr.Name, "TeleportData missing")
                return
        end
        ...
        print(string.format(
                "[Stage5 SessionBootstrap] JobId=%s PlaceId=%d Player=%s sessionId=%s privateCode=%s fromPlaceId=%s reason=%s userRole=%s roomCode=%s",
                game.JobId,
                game.PlaceId,
                plr.Name,
                tostring(sid),
...
Players.PlayerAdded:Connect(function(plr: Player)
        print(string.format(
                "[Stage5 SessionBootstrap] PlayerAdded JobId=%s PlaceId=%d Player=%s",
        ...
        print(("[Stage5 SessionBootstrap] %s sessionId=%s userRole=%s isTeacher=%s roomCode=%s"):format(
                plr.Name,
                tostring(finalSid),
        ...
        -- 3) Stage5 재접속용 Resume 저장
        if typeof(finalSid) == "string" and finalSid ~= "" then
                print(("[Stage5 SessionBootstrap] Save Resume: userId=%d sid=%s stage=%d placeId=%d"):format(
...
print("[Stage5 SessionBootstrap] READY (sessionId + userRole/isTeacher/roomCode + SessionResume + debug)")
