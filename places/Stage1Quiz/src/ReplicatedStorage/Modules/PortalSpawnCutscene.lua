...
--  - ✅ 새 컷씬 시작 시 이전 컷씬(카메라/컨트롤/트윈/딜레이)을 강제 종료하도록 CutsceneManager 토큰 적용
...
function M.play(opts)
	...
	-- ✅ 새 컷씬 시작: 이전 컷씬 강제 종료 + 토큰 생성
	local token = CutsceneManager.Begin("PortalSpawnCutscene")
	...
	token:OnCancel(function()
		...
		if humanoid and humanoid.Parent then
			if oldWalkSpeed ~= nil then humanoid.WalkSpeed = oldWalkSpeed end
			if oldJumpPower ~= nil then humanoid.JumpPower = oldJumpPower end
			if oldJumpHeight ~= nil then humanoid.JumpHeight = oldJumpHeight end
	...
	-- ✅ 파티클은 토큰 딜레이로 (중간에 다른 컷씬 시작되면 자동 무효)
	token:Delay(1, function()
		kickParticles(clone)
	end)
	...
	-------------------------------------------------------
	-- ✅ 카메라 / 이동 복원 (정상 종료도 Cancel 훅으로 통일)
	-------------------------------------------------------
	token:Delay(restoreDelay, function()
		token:Cancel("finish_restore")
	end)
