...
-- [QUIZ][IMPORTANT]
-- RF_Quiz_GetQuestion / RF_Quiz_CheckAnswer 는 "서버에서 생성"되어야 함.
-- 클라에서 Instance.new 로 만들면 서버와 연결 안된 가짜 Remote가 생겨서
-- 문제 안 뜸 / 제출 안 됨 증상이 랜덤으로 발생함.
local RF_Get = Remotes:WaitForChild("RF_Quiz_GetQuestion", 10) :: RemoteFunction
local RF_Check = Remotes:WaitForChild("RF_Quiz_CheckAnswer", 10) :: RemoteFunction

if not RF_Get or not RF_Check then
	warn("[QuizClient] Quiz remotes not found (server not ready)")
	return
end
...
type QDto = {
	...
	q: string,
	title: string?,
	quizStorageId: string,      -- ✅ 추가
	choices: {ChoiceDto},
}

local function safeGetQuestion(solvedList)
	for i = 1, 2 do
		local ok, res = pcall(function()
			return (RF_Get :: RemoteFunction):InvokeServer(solvedList)
		end)

		if ok and typeof(res) == "table" and res.ok == true and typeof(res.data) == "table" then
			return res.data
		end
...
local function openOneQuestion()
	...
	local q = safeGetQuestion(solvedList)
	if not q then
		warn("[QuizClient] question not ready")
		gui.Enabled = false
		busy = false
		return
	end
	...
	--else
	--	-- ✅ 여기서 이유가 보이도록 로그
	--	warn("[QuizClient] RF_Get failed:", res and res.error, res and res.last and res.last.msg)
	--	gui.Enabled = false
	--	busy = false
	--	return
	--end
	...
	-- ✅ choices를 choiceNumber 순서대로 정렬해서 1~4번 버튼에 넣기
	local choices = q.choices or {}
	table.sort(choices, function(a, b)
		return (a.choiceNumber or 0) < (b.choiceNumber or 0)
	end)

	-- 선택한 버튼 index -> quizChoiceId 매핑
	local choiceIdByIndex: {[number]: number} = {}

	for i=1,4 do
		local ch = choices[i]
		if ch then
			lbls[i].Text = tostring(ch.choiceText or "")
			choiceIdByIndex[i] = tonumber(ch.quizChoiceId) or 0
		else
			lbls[i].Text = ""
			choiceIdByIndex[i] = 0
		end

		btns[i].Active = (choiceIdByIndex[i] ~= 0)
	...
	conns[#conns+1] = UI.btnAnswer.MouseButton1Click:Connect(function()
		...
		local result = nil
		local chosenChoiceId = 0
		if selected then
			chosenChoiceId = choiceIdByIndex[selected] or 0
		end

		local ok2, res2 = pcall(function()
			return (RF_Check :: RemoteFunction):InvokeServer(chosenChoiceId, q.quizStorageId)
		end)
