...
local function syncFromQuizState()
	...
	local questIndex = 1
	if phase <= 0 then
		questIndex = 1                 -- 1: NPC에게 말 걸기
	elseif phase == 1 then
		questIndex = 2                 -- 2: 쓰레기 1개 정화 후 말걸기
	elseif phase == 2 then
		questIndex = 3                 -- 3: 쓰레기 9개 정화 시작
	elseif phase == 3 then
		questIndex = 3                 -- 3: 쓰레기 9개 정화 진행중
	elseif phase == 4 then
		questIndex = 4                 -- ✅ 4: NPC에게 말 걸기
	end
	-- questIndex 5 ("다음 스테이지로 이동하기")는
	-- 실제 플레이 중 advanceQuest() 로만 올라가게 두는 구조
	...
	-- 마지막 퀘스트까지 끝났는지는 DataStore에 따로 안 넣었으니까
...
-- ★ QuizState 기반으로 동기화 + 텍스트 세팅 한 번에
local function refreshFromQuizState()
	syncFromQuizState()
	updateQuestText()
end

-- 우선 기본 텍스트만 표시 (QuizState 없을 수도 있으니까)
updateQuestText()
questRoot.Position = BASE_POS_QUEST

-- _QuizState 준비될 때까지 기다렸다가 동기화
task.spawn(function()
	local pg = LP:WaitForChild("PlayerGui")
	-- 여기서 _QuizState 만들어질 때까지 대기
