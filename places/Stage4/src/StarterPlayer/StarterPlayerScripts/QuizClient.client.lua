...
--!strict
-- 스테이지4용 QuizClient (중앙 문 컷씬 + 퀘스트/문제/오브젝트 진행도 서버 연동)
--
-- QuestPhase:
...
-- 중앙 문 컷씬 모듈
local cutScene =
	tryRequire(RS:FindFirstChild("Modules") and RS.Modules:FindFirstChild("MiddleDoorCutscene"))
	or tryRequire(RS:FindFirstChild("MiddleDoorCutscene"))
...
-- ★ Stage4 진행도 동기화용 Remotes
local RF_Stage4_GetProgress   = Remotes:WaitForChild("RF_Stage4_GetProgress")   :: RemoteFunction
local RE_Stage4_QuestSync     = Remotes:WaitForChild("RE_Stage4_QuestSync")     :: RemoteEvent
local RE_Stage4_CutsceneFlag  = Remotes:WaitForChild("RE_Stage4_CutsceneFlag")  :: RemoteEvent
local RE_Stage4_ObjectCleaned = Remotes:WaitForChild("RE_Stage4_ObjectCleaned") :: RemoteEvent
local RE_Stage4_QuizSolved    = Remotes:WaitForChild("RE_Stage4_QuizSolved")    :: RemoteEvent

-- ★ 점수/시간 실시간 저장용(RemoteEvent)
local RE_Stage4_QuizRuntime   = Remotes:WaitForChild("RE_Stage4_QuizRuntime") :: RemoteEvent

-- ========= 상수/태그 =========
local UI_NAME             = "QuizGui"
local TOTAL_QUESTIONS     = 10
local CUTSCENE_THRESHOLD  = 4  -- 4문제 맞추면 중앙 문 컷씬
...
local function reportRuntime(reason: string?)
	if not RE_Stage4_QuizRuntime then return end
	...
	pcall(function()
		RE_Stage4_QuizRuntime:FireServer(payload)
	end)
end
...
local function getCleanedObjectIdFromPrompt(prompt: Instance?): string?
	...
	warn("[Stage4][QuizClient] SessionObjectId not found for prompt:", prompt:GetFullName())
	return nil
end
...
local function localDisappearByInstance(inst: Instance)
	...
	if not ok then
		warn("[Stage4][QuizClient] localDisappearByInstance error:", err)
	end
end
...
local function moveQuestNPCToSpawnOnce(): boolean
	...
	if not (spawn and spawn:IsA("BasePart")) then
		warn("[Stage4][QuizClient] NpcSpawnPart 를 찾지 못했습니다.")
		return false
	end
	...
	if not moved then
		warn("[Stage4][QuizClient] QuestNPC 태그가 붙은 NPC를 찾지 못했습니다.")
	end
	return moved
end
...
local function moveQuestNPCToSpawn()
	...
	while os.clock() < deadline do
		if moveQuestNPCToSpawnOnce() then
			print("[Stage4][QuizClient] NPC 이동 완료(재시작/재입장 포함).")
			return
		end
		task.wait(0.3) -- 0.3초마다 다시 시도
	end
	warn("[Stage4][QuizClient] NPC 이동 재시도 타임아웃(5초) – NpcSpawnPart 또는 QuestNPC 없음.")
end
...
local function syncQuestToServer()
	...
	if not RE_Stage4_QuestSync then return end

	local phase = STATE.QuestPhase.Value
	local extra = STATE.ExtraTrash.Value

	local ok, err = pcall(function()
		RE_Stage4_QuestSync:FireServer(phase, extra)
	end)
	if not ok then
		warn("[Stage4][QuizClient] syncQuestToServer failed:", err)
	end
end
...
local function openOneQuestion()
	if not RF_Get or not RF_Check then
		warn("[Stage4][QuizClient] Remotes missing")
		busy = false
		return
	end
	...
	-- 첫 문제를 여는 순간 시작 시간 초기화
	-- (재입장 복구가 되어 있으면 quizStartedAt 이 이미 세팅되어 있을 수 있음)
	if not quizStartedAt then
		quizStartedAt = os.clock()
		-- totalScore 는 loadStage4Progress 에서 복구될 수 있음
	end
	...
	conns[#conns+1] = UI.btnAnswer.MouseButton1Click:Connect(function()
		...
		if isCorrect then
			...
			task.delay(0.22, function()
				...
				if RE_Stage4_QuizSolved then
					RE_Stage4_QuizSolved:FireServer(qidStr)
				end
				...
				do
					...
					if anchorInst then
						...
						if cleanedObjectId then
							print("[Stage4][QuizClient] cleaned objectId =", cleanedObjectId)
						end
					end
					...
					if cleanedObjectId and RE_Stage4_ObjectCleaned then
						RE_Stage4_ObjectCleaned:FireServer(cleanedObjectId)
					end
				end
				...
				if not cutsceneSent and solved >= CUTSCENE_THRESHOLD then
					...
					if RE_Stage4_CutsceneFlag then
						RE_Stage4_CutsceneFlag:FireServer("middle_door_cut")
					end
					...
					task.delay(1.2, function()
						...
						if cutScene and typeof(cutScene.Play) == "function" then
							pcall(function() cutScene.Play() end)
						end
					end)
				end
				...
				if solved >= TOTAL_QUESTIONS then
					...
					if RE_StageQuizResult then
						...
						warn(("[Stage4][QuizClient] RE_StageQuizResult missing; score=%d, time=%ds not sent")
...
ProximityPromptService.PromptTriggered:Connect(function(prompt: ProximityPrompt, player: Player)
	...
	if isDialogueStartPrompt(prompt) then
		...
		if not DialogueBus then
			warn("[Stage4][QuizClient] DialogueUIBus 를 찾지 못했습니다.")
			busy = false
			currentPrompt = nil
			return
		end
...
local function waitForSessionId(timeoutSec: number?)
	...
	while os.clock() < deadline do
		...
		if typeof(sid) == "string" and sid ~= "" then
			print("[Stage4][QuizClient] sessionId ready:", sid)
			return
		end
		task.wait(0.1)
	end
	warn("[Stage4][QuizClient] sessionId not set in time")
end

local function loadStage4Progress()
	isLoadingProgress = true

	local ok, res = pcall(function()
		return RF_Stage4_GetProgress:InvokeServer()
	end)
	if not ok or typeof(res) ~= "table" then
		warn("[Stage4][QuizClient] RF_Stage4_GetProgress failed:", res)
	...
	print("[Stage4][QuizClient] progress loaded. solved qids:", SolvedQuiz, "cutscenes:", CutsceneFlags)
	...
	if solvedCount > 0 then
		...
		Hud.Show(TOTAL_QUESTIONS)
		Hud.Progress(STATE.Solved.Value, TOTAL_QUESTIONS)
		-- 필요하면 아래 라인 풀어서 "이미 맞힌 수"만큼 HUD에 연두색으로 반영 가능
		-- Hud.Correct(STATE.Solved.Value, TOTAL_QUESTIONS)
	end

	-------------------------------------------------------
	-- 점수/시간 복구 (서버 스키마: quizScore / quizTimeSec)
	-------------------------------------------------------
	local savedScore = tonumber(res.quizScore)
	local savedTime  = tonumber(res.quizTimeSec)

	if savedScore and savedScore > 0 then
		totalScore = savedScore
	end
	if savedTime and savedTime > 0 then
		-- 이전까지 걸린 시간(savedTime)에서 다시 이어서 측정
		quizStartedAt = os.clock() - savedTime
	end
	...
	print(("[Stage4][QuizClient] ExtraTrash restored: saved=%d, derived=%d, final=%d")
			...
			if flag then
				task.spawn(function()
					...
					while os.clock() < deadline and not removed do
						local inst = findBySessionObjectId(objectId)
						if inst then
							print("[Stage4][QuizClient] auto-clean local object:", objectId, inst:GetFullName())
							localDisappearByInstance(inst)
							removed = true
							break
						end
					...
					if not removed then
						warn("[Stage4][QuizClient] cleaned object not found for id:", objectId)
					end
				end)
			end
		end
	...
	-------------------------------------------------------
	-- 컷씬 플래그 (중앙 문 컷씬 재입장 처리)
	-------------------------------------------------------
	if CutsceneFlags["middle_door_cut"] or CutsceneFlags["jelly_cut"] then
		print("[Stage4][QuizClient] middle door cut already done → skip full cutscene")
		STATE.ZoomPlayed.Value = true
		cutsceneSent = true

		-- 재입장 시: NPC 위치만 옮기고, 문만 다시 움직이게
		pcall(moveQuestNPCToSpawn)

		task.delay(1.0, function()
			if cutScene and typeof(cutScene.PlayDoorOnly) == "function" then
				pcall(function()
					cutScene.PlayDoorOnly()
				end)
			end
		end)
	end
...
print("[Stage4][QuizClient] READY (퀘스트0~3, 쓰레기10개 정화, 4문제 중앙 문 컷씬, NPC/포탈 가이드 + Score/Time + 진행도 복구 + 중간 점수/시간 저장)")
