...
-- ★ 돌고래 초상 이미지
local DOLPHIN_PORTRAIT_ID = "rbxassetid://128875011792933"
...
local SFX_IDS: {[string]: string} = {
	...
	DialogueStart = "rbxassetid://17417730290",         -- 캐릭터/대화 등장
...
local function buildUI(): ScreenGui
	...
	gui.Enabled = true -- ✅ ScreenGui는 항상 켜두기 (표시는 BG로)
	gui.Parent = pg
...
local function runDialogueSequence(npc: Instance?, src:any, questPhase:number?)
	...
	local function handleAdvance(playNextSound: boolean)
		...
		if (not isHintLine)
			...
			and string.find(currentText, "고마우니까 내가 다음 구역 포탈 열어줄게! 받아라, 집게 파워", 1, true) ~= nil
		then
