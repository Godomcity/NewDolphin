...
--  - 문은 이미 열린 상태 (MiddleDoorCutscene.PlayDoorOnly 사용)
--  - NPC를 최종 위치로 이동
...
-- ★ 문 연출용 MiddleDoorCutscene 모듈 (PortalMover 대신 사용)
local MiddleDoorCutscene =
...
-- ★ PortalMover 대신 MiddleDoorCutscene.PlayDoorOnly 사용
local function applyDoorOpenedVisual()
	if not MiddleDoorCutscene or typeof(MiddleDoorCutscene.PlayDoorOnly) ~= "function" then
		warn("[StageTeacherSkip_Stage1] MiddleDoorCutscene.PlayDoorOnly 를 찾지 못했습니다. 문 오픈 스킵")
		return
	end

	local ok, err = pcall(function()
		MiddleDoorCutscene.PlayDoorOnly()
	end)

	if not ok then
		warn("[StageTeacherSkip_Stage1] applyDoorOpenedVisual (PlayDoorOnly) failed:", err)
	end
end
