...
local function actionReplaceWithReplicatedFirst(root: Instance, ctx)
	...
	-- 1) 교체 대상(Model 또는 Part)
	local target = root
	local parent = target.Parent
	if not parent then
		warn("[ActionRouter] parent 없음")
		return
	end
	...
	-- 2) 원본의 BoundingBox 중심 CFrame 추출
	local originalCF: CFrame
	if target:IsA("Model") then
		originalCF = (target :: Model):GetBoundingBox()
	elseif target:IsA("BasePart") then
		originalCF = (target :: BasePart).CFrame
	else
		warn("[ActionRouter] 지원되지 않는 타입:", target.ClassName)
		return
	end

	-- 3) 템플릿 찾기
        local template = ReplicatedFirst:FindFirstChild(target.Name)
        if not template then
                warn("[ActionRouter] 템플릿 없음:", target.Name)
                return
        end
	...
	-- 5) 템플릿 자체 BoundingBox 계산 (중심 피벗)
	local templateCF: CFrame
	if clone:IsA("Model") then
		templateCF = (clone :: Model):GetBoundingBox()
	elseif clone:IsA("BasePart") then
		templateCF = (clone :: BasePart).CFrame
	end

	-- ★ 핵심: 템플릿 중심 → 원본 중심으로 강제 이동
	-- clone 전체 이동: originalCF * templateCF:Inverse() 를 사용
	local delta = originalCF * templateCF:Inverse()
