...
local RF_Stage1_GetProgress   = ensureRF("RF_Stage4_GetProgress")
local RE_Stage1_QuestSync     = ensureRE("RE_Stage4_QuestSync")
local RE_Stage1_CutsceneFlag  = ensureRE("RE_Stage4_CutsceneFlag")
local RE_Stage1_ObjectCleaned = ensureRE("RE_Stage4_ObjectCleaned")
local RE_Stage1_QuizSolved    = ensureRE("RE_Stage4_QuizSolved") -- ★ 퀴즈 정답 보고
local RE_Stage1_QuizRuntime   = ensureRE("RE_Stage4_QuizRuntime") -- ★ 점수/시간 실시간 저장
...
--------------------------------------------------
RE_Stage1_QuizRuntime.OnServerEvent:Connect(function(plr: Player, payload: any)
	local key = getKey(plr)
	if not key then return end

	-- 클라이언트에서 { score=number, timeSec=number, solved=number, reason=string } 테이블로 옴
	local score   = 0
	local timeSec = 0

	if typeof(payload) == "table" then
		score   = tonumber(payload.score)   or 0
		timeSec = tonumber(payload.timeSec) or 0
	else
		-- 혹시 예전 방식(숫자 2개)으로도 들어올 수 있으니 방어적으로 처리
		score   = tonumber(payload) or 0
	end

	if score   < 0 then score   = 0 end
	if timeSec < 0 then timeSec = 0 end

	print(("[Stage4ProgressService] QuizRuntime plr=%s key=%s score=%d time=%ds")
