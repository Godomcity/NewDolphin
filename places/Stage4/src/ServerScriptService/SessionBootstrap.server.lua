...
local function debugPrintJoinData(plr: Player, joinData: any?)
        if typeof(joinData) ~= "table" then
                print("[Stage4 SessionBootstrap]", plr.Name, "JoinData missing")
                return
        end
        ...
        print(string.format(
                "[Stage4 SessionBootstrap] JobId=%s PlaceId=%d Player=%s sessionId=%s privateCode=%s fromPlaceId=%s reason=%s userRole=%s roomCode=%s",
                game.JobId,
                game.PlaceId,
                plr.Name,
                tostring(sid),
...
Players.PlayerAdded:Connect(function(plr: Player)
        ...
        print(("[Stage4 SessionBootstrap] %s sessionId=%s userRole=%s isTeacher=%s roomCode=%s"):format(
                plr.Name,
                tostring(finalSid),
...
print("[Stage4 SessionBootstrap] READY (sessionId + userRole/isTeacher/roomCode + SessionResume + debug)")
