...
-- + Stage2에서는 SessionResume에 "지금 Stage2에 있다" 라는 정보도 저장
...
local function debugPrintJoinData(plr: Player, joinData: any?)
        if typeof(joinData) ~= "table" then
                print("[Stage2 SessionBootstrap]", plr.Name, "JoinData missing")
                return
        end

        local td = joinData.TeleportData
        if typeof(td) ~= "table" then
                print("[Stage2 SessionBootstrap]", plr.Name, "TeleportData missing")
                return
        end
        ...
        print(string.format(
                "[Stage2 SessionBootstrap] JobId=%s PlaceId=%d Player=%s sessionId=%s privateCode=%s fromPlaceId=%s reason=%s userRole=%s roomCode=%s",
                game.JobId,
                game.PlaceId,
                plr.Name,
                tostring(sid),
...
Players.PlayerAdded:Connect(function(plr: Player)
        print(string.format(
                "[Stage2 SessionBootstrap] PlayerAdded JobId=%s PlaceId=%d Player=%s",
        ...
        print(("[Stage2 SessionBootstrap] %s sessionId=%s userRole=%s isTeacher=%s roomCode=%s"):format(
                plr.Name,
                tostring(finalSid),
