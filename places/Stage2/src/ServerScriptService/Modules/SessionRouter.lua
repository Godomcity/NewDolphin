...
-- 동시 호출 시 서로 다른 코드가 생기지 않도록 UpdateAsync + pending 락 사용
...
function M.GetOrCreate(sessionId: string, targetPlaceId: number): (boolean, string?, string?)
	...
	-- 3) 나는 락 주인이 아님 → 다른 서버가 코드 만드는 중이니 잠깐 대기
	for _ = 1, 10 do -- 최대 ~2초
		task.wait(0.2)
